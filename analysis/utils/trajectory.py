...
def agentdf_to_dfic(df):
    ...
    dfic["agent"] = df["agent_attributes"].apply(
        lambda x: str(x)
    )  # use agent dict as str as unique agent kind identifier
    dfic["discreteWorld"] = df["blockmap"].apply(lambda x: (x > 0).astype(int))
    # from preprocessing in jupyter notebook
    dfic["usableDiscreteWorld"] = dfic["discreteWorld"].apply(
        lambda a: 1 + (-1) * np.array(a)
    )
    dfic["flatDiscreteWorld"] = dfic["discreteWorld"].apply(
        lambda a: (1 + (-1) * np.array(a)).flatten()
    )
...
worlds_silhouettes_legal = {
    "int_struct_legal_" + str(i): s for i, s in silhouettes.items()
}
worlds_small = {
...
# with open(os.path.join(main_dir,'results/csv/targetMaps.txt')) as json_file:
...
def bootstrapCI(x, nIter=1000, crit_val=0, ci=95, verbose=False, compareNull=True):
    """
    input: x = an array,
    ...
    p1 = (
        len([i for i in u if i < crit_val]) / len(u) * 2
    )  ## first version of p-value reflects number of samples that have value below crit_val
    p2 = (
        len([i for i in u if i > crit_val]) / len(u) * 2
    )  ## second version of p-value reflects number of samples that have value above crit_val
    U = np.mean(u)
    ...
    if verbose:
        print(
            "Original mean = {}. Bootstrapped mean = {}.".format(
                np.mean(x).round(5), U.round(5)
            )
        )
        ...
        if compareNull:
            print(
                "p<{}={} | p>{}={}.".format(
                    crit_val, np.round(p1, 5), crit_val, np.round(p2, 5)
                )
            )
...
def statsPrint(x, nIter=1000, crit_val=0, ci=95, verbose=True, compareNull=True):
...
class GenericNode:
    ...
    def __init__(self, discrete_world, f1_score, target_name):
        ...
        self.precision = scoring.get_precision(target_maps[target_name], discrete_world)
        ...
        if num_squares > 0:
            proportion_filled = (
                num_squares / 64
            )  # proportion of grid-world (8x8) filled
        else:
            proportion_filled = 0
        ...
        self.x = ndimage.measurements.center_of_mass(
            discrete_world[:, condsider_mass_above:]
        )[0]
...
class GenericBuildGraph:
    def __init__(self, target_name="no_target_name_given"):
        ...
        self.world_layers[0] = GenericWorldLayer(
            0
        )  # layers to allow quick indexing of world state
        # create root in correct shape for target
        target_shape = target_maps[target_name].shape
        # Add layer for every multiple of two, or add manually?
        self.root = self.world_layers[0].get_world_node(
            np.zeros(target_shape).astype(int), 0, target_name
        )  # start with one node with empty world
    ...
    def add_block_placement(self, row):
        ...
        if world_squares not in self.world_layers:  # if layer doesn't exist, add layer
        ...
        new_node = layer.get_world_node(
            row.discreteWorld, row.rawF1DiscreteScore, self.target_name
        )
...
def plot_trajectory_graph(
    ...
    def make_edge(x, y, width):
        return go.Scatter(
            x=x, y=y, line=dict(width=width), hoverinfo="none", mode="lines"
        )
        ...
        for i in range(0, int(len(edge_xs) / 2)):
            edge_trace.append(
                go.Scatter(
                    ...
                    line=dict(
                        width=edge_ws[i] * edge_width_scale_factor,
    ...
    fig.update_yaxes(
        tickfont=dict(family="Helvetica", color="black", size=24),
        title_text="F1 score",
    ...
    if save:
        ...
        plot_path = os.path.join(
            out_dir,
            "state_trajectory",
            (target_name + "_" + agent + "_" + extension + ".pdf"),
        )
...
def get_sparsity_over_states(
    data=[], target="hand_selected_004", phase="pre", metric="entropy"
):
    """

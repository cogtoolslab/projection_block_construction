...
def agentdf_to_dfic(df):
    ...
    dfic['agent'] = df['agent_attributes'].apply(lambda x: str(x)) #use agent dict as str as unique agent kind identifier
...
class GenericBuildGraph:
    def __init__(self, target_name='no_target_name_given'):
        ...
        self.root = self.world_layers[0].get_world_node(np.zeros(target_shape).astype(int), 0, target_name) # start with one node with empty world

...
def fill_average_cost_per_step(df):
    for run_ID in df['run_ID'].unique():
        ...
        steps = len(slice_df)
        df.loc[(df['run_ID'] == run_ID) & (df['final_row'] == True),'avg_cost_per_step_for_run'] = cost/steps
...
def add_increment_sequence(table):
    ...
    for run_ID in table['run_ID'].unique():
        try:
            increments = table.query("run_ID == @run_ID").sort_values('step')['decomposition_increment']
            increments = [i for i in increments if not pd.isna(i)]
            #write increments back to df
            table.loc[table.run_ID == run_ID,'decomposition_increment_sequence'] = str(increments)
        except KeyError:

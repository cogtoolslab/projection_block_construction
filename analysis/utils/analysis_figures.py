...
def build_animation(slice, title=None):
    ...
    try:
        ...
        decomposed_silhouettes = [
            s for s in decomposed_silhouettes if type(s) is not float
        ]  # remove nans (which are floats)
    except KeyError:
        decomposed_silhouettes = []
    ...
    try:
        ...
        plans = [
            [g["name"] for g in s] if type(s[0]) is dict else [g["name"] for g in s[0]]
            for s in plans
            if type(s) is not float
        ]  # get list of names for the plans that aren't empty
        # TODO why are some subgoal sequences wrapped in an additional list?
    except KeyError:
        plans = []
    if plans == []:
        plans = [[8]]  # if we dont have a plan the plan is full decomp
    plan_counter = 0
    frame = 0
    for i, row in slice.iterrows():
        ...
        if (
            type(row["world_failure_reason"]) is not float
            and row["world_failure_reason"] != "None"
        ):
            plt.text(
                4,
                4,
                s=row["world_failure_reason"],

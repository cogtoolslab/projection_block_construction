...
def get_scored_sequences(list_of_list_of_sequences: List) -> OrderedDict:
    ...
    if (
        len(list_of_list_of_sequences) == 0
        or sum([len(s) for s in list_of_list_of_sequences]) == 0
    ):
        raise Warning("No solved sequences")
        return None
    # initialize scores
    sequence_scores = {
        name: 0 for name in [s.names() for s in list_of_list_of_sequences[0]]
    }
    sequence_complete = {
        name: True for name in [s.names() for s in list_of_list_of_sequences[0]]
    }
    # compute scores
    for list_of_sequences in list_of_list_of_sequences:
        for sequence in list_of_sequences:
            sequence_scores[sequence.names()] += sequence.planning_cost()
            sequence_complete[sequence.names()] = (
                False
                if not sequence.complete()
                else sequence_complete[sequence.names()]
            )
    sequence_filtered = {
        name: score
        for name, score in sequence_scores.items()
        if sequence_complete[name]
    }
    # sort the dictionary
    sequence_sorted = OrderedDict(
        sorted(sequence_filtered.items(), key=lambda pair: pair[1])
    )
...
def filter_sequences(list_of_list_of_sequences: List[list]) -> List[list]:
    ...
    return [
        [
            ...
            if len(seq.subgoals) <= MAX_SUBGOALS and seq.names()[-1] == FINAL_HEIGHT
        ]
        for list_of_sequences in list_of_list_of_sequences
    ]
...
def permutations(names: Tuple) -> Set:
    ...
    permutated_names = [
        tuple([abs(sum(pi[0:i]) + pi[i]) for i in range(len(pi))])
        for pi in permutated_increments
    ]
    permutated_names = set(
        [pn for pn in permutated_names if pn != names]
    )  # remove duplicates and original sequence
...
def extract_best_and_worst_sequence(sequence_scores: OrderedDict):
    ...
    for best_name, best_score in sequence_scores.items():
        ...
        sequence_permutations = {
            p_name: sequence_scores[p_name]
            for p_name in best_permutations
            if p_name in sequence_scores.keys()
        }
        # sort permutations
        sequence_permutations = OrderedDict(
            sorted(sequence_permutations.items(), key=lambda pair: pair[1])
        )
...
def extract_most_divergent_pair_sequence(sequence_scores: OrderedDict):
    ...
    # sort pair_scores
    # print(pair_scores)
    pair_scores = OrderedDict(
        sorted(
            pair_scores.items(), key=lambda pair: pair[1][1] - pair[1][0], reverse=True
        )
    )

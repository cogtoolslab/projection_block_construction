...
if __name__=="__main__": #required for multiprocessing
    ...
    generator = tower_generator.TowerGenerator(4,4,#8, 8,
    ...
    # actually run it
    results = simulated_subgoal_planner_experiment_runner.run_experiment(superset_results,agents,per_exp=1,steps=32,parallelized=FRACTION_OF_CPUS,save=EXP_NAME+"_simulated_results",maxtasksperprocess = 1)
    print("Done in %s seconds" % (time.time() - start_time))
    print(f"Generated {len(results)} lines of data from the simulated subgoal agents")
    print("Files saved to {} with postfixes".format(os.path.join(proj_dir,'experiments','results',EXP_NAME)))
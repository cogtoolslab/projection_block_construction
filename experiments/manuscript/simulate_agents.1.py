...
FILE_BY_FILE = (
    True  # if true, only load the df needed to memory and save out incremental results
)
    ...
    for l in LAMBDAS:
        l_agents = [
            Simulated_Subgoal_Agent(
                c_weight=l, decomposer=no_subgoals_decomposer, label="No Subgoals"
            ),
            Simulated_Subgoal_Agent(
                c_weight=l, decomposer=myopic_decomposer, label="Myopic"
            ),
            Simulated_Subgoal_Agent(
                c_weight=l, decomposer=lookahead_1_decomposer, label="Lookahead 1"
            ),
            Simulated_Subgoal_Agent(
                c_weight=l, decomposer=lookahead_2_decomposer, label="Lookahead 2"
            ),
        ]
        agents += l_agents

...
if __name__ == "__main__":  # required for multiprocessing
    ...
    window_sizes = [4, 8, 12, 16, 20, 24, 32] # 32 is maximum area that we can cover
    ...
    scoping_agents = [Subgoal_Planning_Agent(lower_agent=lower_agent,
                                             ...
                                             label="Incremental Scoping max size={} lambda={}".format(ms, cw)) for ms in window_sizes for cw in lambdas]
    ...
    lookahead2_agents = [Subgoal_Planning_Agent(lower_agent=lower_agent,
                                                ...
                                                step_size=1,
                                                max_number_of_sequences=8192,
                                                label="Lookahead Scoping max size={} lambda={}".format(ms, cw)) for ms in window_sizes for cw in lambdas]

    print("Running experiment...")
    results_sg = experiment_runner.run_experiment(
        worlds,
        [*scoping_agents],
        per_exp=1, #TODO set back to 10
        steps=16,
        verbose=False,
        parallelized=fraction_of_cpus,
        save="RLDM_scoping_window_size_incremental_bigtowers_experiment",

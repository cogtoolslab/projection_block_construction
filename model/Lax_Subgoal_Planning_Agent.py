...
class Lax_Subgoal_Planning_Agent(Subgoal_Planning_Agent):
    ...
    def score_subgoals_in_sequence(self,sequences,verbose=False):
        ...
        for sequence in sequences: #reference or copy?
            ...
            for subgoal in sequence:
                ...
                S,C,post_world,total_cost,stuck = self.success_and_cost_of_subgoal(subgoal['decomposition'],prior_world, iterations=self.S_iterations)
                R = self.reward_of_subgoal(post_world.current_state.blockmap,prior_world.current_state.blockmap) 
    ...
    def reward_of_subgoal(self,post_blockmap,prior_blockmap):
        ...
        return np.sum(((post_blockmap > 0) * self.world.full_silhouette) - (prior_blockmap > 0))

    def success_and_cost_of_subgoal(self,decomposition,prior_world = None, iterations=1,max_steps = 20):

...
class BFS_Lookahead_Agent(Agent):
        ...
        for i in range(horizon):  # this just idles when no children are to be found
            ...
            for node in current_nodes:
                ...
                for action in node.actions:
                    self.score_node(action.target, self.sparse,
                                    self.dense_stability)  # score the node
                ...
                children_nodes += [
                    action.target for action in node.actions if node.stability is not False]
            current_nodes = children_nodes
            if verbose:
                print("Depth of AST:", i+1, ",found",
                      len(current_nodes), "nodes")
    ...
    def score_action_sequences(self, action_sequences, method='Final_state', verbose=False):
        ...
        for act_seq in action_sequences:  # score every action sequence
            if method == 'Final_state':
                score = act_seq.actions[-1].target.score + (
                    not act_seq.actions[-1].target.stability) * self.world.fail_penalty
            elif method == 'Sum':
                score = []
                for action in act_seq.actions:
                    score.append(
                        action.target.score + (not action.target.stability) * self.world.fail_penalty)
                score = sum(score)
            elif method == 'Average':
                score = []
                counter = 0
            ...
            act_seq.score = score
            if verbose:
                print([a.action.__str__()
                      for a in act_seq.actions], " score: ", score)
    ...
    class Action_sequence():
        ...
        def print_actseq(self):
            print("Action sequence:", [[str(b) for b in act.action]
                  for act in self.actions], "Score:", self.score)

    def select_action_seq(self, scored_actions):  # could implement softmax here
        max_score = max(
            [action_seq.score for action_seq in scored_actions if action_seq.score is not None])
    ...
    def act(self, steps=None, planning_horizon=None, verbose=False, scoring=None):
        ...
        if planning_horizon < steps:
            print("Planning horizon must be higher or equal to the steps or 0! Setting the horizon from",
                  planning_horizon, "to", steps)
            planning_horizon = steps
        ...
        if self.first_solution and win_seq is not None:
            ...
            if verbose:
                ...
                print("Chosen action sequence:", [
                      [str(b) for b in a.action] for a in chosen_seq.actions], "with score: ", chosen_seq.score)
        # take the steps
        ...
        for step in range(min([steps, len(chosen_seq.actions)])):
            ...
            if verbose:
                print("Took step ", step+1, " with action ", [str(
                    a) for a in chosen_seq.actions[step].action], " and got world state", self.world.current_state)
    ...
    def add_action(self, action, target=None):
        if action in [act.action for act in self.actions]:
            ...
            if target is None:  # figure out the target state
                target = Ast_node(
                    self.state.world.transition(action, self.state))

...
import scoping_simulations.utils.blockworld_helpers as bw
import json
...
def add_block_to_world(block, b2world, SIZE_FACTOR=1, Y_SHIFT=1):
    ...
    body = b2world.CreateDynamicBody(
        position=(b2_x(block)*SIZE_FACTOR, b2_y(block)*SIZE_FACTOR*Y_SHIFT))
    world_block = body.CreatePolygonFixture(box=(
        (block.width/2)*SIZE_FACTOR, (block.height/2)*SIZE_FACTOR), density=1, friction=0.3)
    ...
    while True:
        ...
        if step == 80:  # need to do this not with step?
            start_positions = np.array(
                [body.position for body in b2world.bodies])
...
def render_b2step(b2world,
    ...
    for body in b2world.bodies:
        for fixture in body.fixtures:
            ...
            vertices = [(body.transform * v) * PPM /
                        SIZE_FACTOR for v in shape.vertices]
            vertices = [(v[0] + DISPLAY_OFFSET_X, SCREEN_HEIGHT -
                         v[1] + DISPLAY_OFFSET_Y) for v in vertices]
    ...
    game_display.blit(
        text, (SCREEN_WIDTH - 30 - text.get_width(), SCREEN_HEIGHT - 50))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--SCREEN_WIDTH', type=int,
                        help='screen width', default=400)
    parser.add_argument('--SCREEN_HEIGHT', type=int,
                        help='screen height', default=400)
    parser.add_argument('--DISPLAY_OFFSET_X', type=int,
                        help='horizontal display offset', default=100)
    parser.add_argument('--DISPLAY_OFFSET_Y', type=int,
                        help='horizontal display offset', default=-100)
    parser.add_argument('--TIME_STEP', type=float,
                        help='the length of time passed to simulate (seconds)', default=0.01)
    parser.add_argument('--VEL_ITERS', type=int,
                        help='how strongly to correct velocity', default=10)
    parser.add_argument('--POS_ITERS', type=int,
                        help='how strongly to correct position', default=10)
    parser.add_argument('--TEST_NAME', type=str,
                        help='which test do you want to run? options: T, stonehenge, jenga', default='jenga')
    parser.add_argument('--BLOCKS_REMOVED', type=int,
                        help='how many blocks to remove? only applies to jenga test.', default=5)
    parser.add_argument('--SIZE_FACTOR', type=float,
                        help='scale of blocks in physics engine. 1: 1 meter per unit. Less than 0.5 unstable', default=1)
    parser.add_argument('--RENDER', type=str,
                        help='display blocks with pygame', default='true')
    parser.add_argument('--TEST_TIME_STEPS', type=int,
                        help='number of box2d iterations before stability confirmed', default=100)
    parser.add_argument('--Y_SHIFT', type=float,
                        help='vertical offset of each block when added to world', default=1)

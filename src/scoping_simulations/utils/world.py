class World:
    ...
    def __init__(self):
        ...
        self.action_space = (
            self.initial_state.keys
        )  # setting up the action space as  the parts of the world state
    ...
    def transition(self, action, state=None):
        ...
        if self.is_fail(state) or self.is_win(
            state
        ):  # if we're in a final state we just return the final state
            return state
        # apply action
        try:
            state[action] = 1
    ...
    def possible_actions(
        self, state=None
    ):  # returns actions whose corresponding worldstate is not 1 yet
        if state is None:
            state = self.world_state
        if self.is_fail(state) or self.is_win(
            state
        ):  # if we're in a final state there's no actions to take
            return []

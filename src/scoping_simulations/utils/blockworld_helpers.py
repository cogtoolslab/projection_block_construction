"""Taken from /block_construction/stimuli/blockworld_helpers.py"""
...
def get_patch(verts, color="orange", line_width=2):
    """
    input:
...
def render_blockworld(patches, xlim=(-2, 10), ylim=(-2, 10), figsize=(4, 4)):
    """
    input:
...
def generate_random_world(
    remove_num_blocks=5, block_dims=[(1, 2), (2, 1), (2, 2), (2, 4), (4, 2)]
):
...
def save_world_json(
    block_dict, path_to_dump="./sampled_worlds_json", worldId=123456789
):
    """
    ...
    with open(
        os.path.join(
            path_to_dump,
            "blockworld_area{}_num{}_time{}.js".format(total_area, num_blocks, worldId),
        ),
        "w",
    ) as fout:
...
def save_world_render(
    block_dict, path_to_dump="./sampled_worlds_render", worldId=123456789
):
    """
    ...
    fig.savefig(
        os.path.join(
            path_to_dump,
            "blockworld_area{}_num{}_time{}.png".format(
                total_area, num_blocks, worldId
            ),
        )
    )
    plt.close(fig)
...
def save_world(block_dict, path_to_dump="./sampled_worlds"):
    ...
    block_dict = save_world_json(
        block_dict, worldId=worldId, path_to_dump=path_to_dump + "_json"
    )
    save_world_render(
        block_dict, worldId=worldId, path_to_dump=path_to_dump + "_render"
    )
...
class World:
    ...
    def __init__(
        ...
        block_dims=[
            (
                1,
                1,
            ),  # Blocks from left to right, thinest to thickest, shortest to tallest
            (1, 2),
            (2, 1),
            (2, 2),
            (2, 4),
        ...
        self.base_blocks = [
            BaseBlock(w, h, color=c)
            for ((w, h), c) in list(
                zip(sorted(block_dims), block_colors[0 : len(block_dims)])
            )
        ]  # Block types should be in order from left to right, thinest to thickest, shortest to tallest
        ...
        self.blockmap = np.zeros((self.world_width, self.world_height), dtype=int)
        self.blocks = []
        self.full = False
    ...
    def fill_floor(self, floor_space):
        ...
        while remaining_space > 0:
            ...
            if self.block_widths[i] <= remaining_space:
                floor_blocks.append(
                    [self.base_blocks[i], floor_space - remaining_space]
                )
    ...
    def fill_floor_here(self, current_level, left_lim, right_lim):
        ...
        copy.deepcopy(self.block_widths)
        ...
        while remaining_space > 0 and len(viable_blocks) > 0:
            ...
            if (
                base_block.width <= remaining_space
                and base_block.height <= remaining_height
            ):
    ...
    def _update_map_with_blocks(self, blocks, delete=False):
        ...
        for b in blocks:
            self.blockmap[
                self.world_height - (b.y + b.height) : self.world_height - b.y,
                b.x : (b.x + b.width),
            ] = new_number

    def can_place(self, block):
        overlap = self.blockmap[
            self.world_height - (block.y + block.height) : self.world_height - block.y,
    ...
    def fill_world(self, render=False):
        ...
        while current_level <= self.world_height - 1:  # Until at top
            # find floor
            # check if level is full or reached top
            while (
                self.blockmap[self.world_height - current_level - 1].all()
                and current_level < self.world_height
            ):
                current_level += 1
            ...
            while (
                ...
                and self.blockmap[self.world_height - current_level - 1][right] == 0
            ):
                right += 1
            # print('fill_world_here: ' + str((current_level, left, right)))
            # fills world and returns whether world changed
            if not self.fill_floor_here(current_level, left, right):
                break
    ...
    def save_to_json(self):
        ...
        return json.dumps({"blocks": block_string})
    ...
    def jenga_block(self, block_number, render=False, checking=False):
        ...
        if block_number < len(self.blocks):
            ...
            for b2 in updated_blocks[block_number:]:
                # block stable if on floor (and avoids indexing errors)
                if b2.y > 0:
                    # get x loc of base of block in blockmap
                    xs = list(range(b2.x, b2.x + b2.width))
                    # get the floor under block
                    support = new_blockmap[(self.world_height - 1) - (b2.y) + 1, xs]
                    # support is the space underneath the base of a block
                    # stable if greater than half of support is 1 in blockmap
                    stable = stable and (np.mean(support) >= 0.5)

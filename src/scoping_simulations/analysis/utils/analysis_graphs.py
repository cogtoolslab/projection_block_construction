...
def mean_win_per_agent(df):
    ...
    plt.bar(
        np.arange(len(scores)), scores, align="center", label=agent_labels(agents, df)
    )
    plt.xticks(
        np.arange(len(scores)), agent_labels(agents, df), rotation=45, ha="right"
    )
    plt.ylim(0, 1)
...
def mean_failure_reason_per_agent(df, fast_fail=False):
    ...
    scores = [
        mean_failure_reason(
            df[
                (df["agent_attributes"] == a)
                & (df["world_status"].isin(["Fail", "Ongoing"]))
            ],
            "Full",
        )
        for a in agents
    ...
    plt.bar(
        np.arange(len(scores)) + 0, scores, align="center", label="Full", width=0.15
    )
    # Unstable
    scores = [
        mean_failure_reason(
    ...
    if fast_fail:
        # Outside
        scores = [
            mean_failure_reason(df[df["agent_attributes"] == a], "Outside")
            for a in agents
        ]
        plt.bar(
            np.arange(len(scores)) + 0.45,
            scores,
            align="center",
            label="Outside",
            color=FAIL_COLOR,
        ...
        # Holes
        scores = [
            mean_failure_reason(df[df["agent_attributes"] == a], "Holes")
            for a in agents
        ]
        plt.bar(
            np.arange(len(scores)) + 0.6,
            scores,
            align="center",
            label="Holes",
            color="red",
...
def avg_steps_to_end_per_agent(df):
    ...
    results = [
        avg_steps_to_end(
            df[(df["run_ID"].isin(run_IDs)) & (df["agent_attributes"] == a)]
        )
        for a in agents
    ]
    scores = [score for score, std in results]
...
def mean_score_per_agent(df, scoring_function=bw.F1score):
    ...
    # all
    results = [
        mean_score(df[df["agent_attributes"] == a], scoring_function) for a in agents
    ]
    scores = [score for score, std in results]
    stds = [std for score, std in results]
    plt.bar(
...
def mean_peak_score_per_agent(df, scoring_function=bw.F1score):
    ...
    # all
    results = [
        mean_peak_score(df[df["agent_attributes"] == a], scoring_function)
        for a in agents
    ]
    scores = [score for score, std in results]
    stds = [std for score, std in results]
...
def mean_num_subgoals_per_agent(df):
    ...
    # all
    scores = [
        sum(~df[df["agent_attributes"] == a]["decomposed_silhouette"].isna())
        / len(df[df["agent_attributes"] == a]["run_ID"].unique())
        for a in agents
    ]
    ...
    scores = [
        sum(
            ~df[(df["agent_attributes"] == a) & (df["run_ID"].isin(run_IDs))][
                "decomposed_silhouette"
            ].isna()
        )
        / len(
            df[(df["agent_attributes"] == a) & (df["run_ID"].isin(run_IDs))][
...
def mean_avg_area_under_curve_to_peakF1_per_agent(df):
    ...
    results = [
        mean_avg_area_under_curve_to_peakF1(
            df[df["agent_attributes"] == a], scoring_function
        )
        for a in agents
    ]
    scores = [score for score, std in results]
...
def graph_mean_F1_over_time_per_agent(df):
    ...
    for a, agent in enumerate(agents):  # plot per agent
        ...
        for i, row in enumerate(a_runs):  # for each run of the agent
            ...
            scores = [
                scores[i] if i < len(scores) else scores[-1] for i in range(PADDING + 1)
            ]
...
def graph_avg_blocksize_over_time_per_agent(df):
    ...
    for a, agent in enumerate(agents):  # plot per agent
        ...
        for run in a_runs:  # for each run of the agent
            ...
            # append (pad) score with last value to xlim as a way of handling the early termination of trials
            scores = [
                scores[i] if i < len(scores) else np.nan for i in range(PADDING + 1)
            ]
            run_scores.append(scores)
        # avg,std
...
def mean_touching_last_block_per_agent(df):
    ...
    # all
    results = [
        touching_last_block_score(df[df["agent_attributes"] == a]) for a in agents
    ]
    scores = [score for score, std in results]
    stds = [std for score, std in results]
    plt.bar(
...
def mean_pairwise_raw_euclidean_distance_between_runs(df):
    ...
    results = [
        [
            pairwise_raw_euclidean_distance_between_blocks_across_all_runs(
                df[(df["agent_attributes"] == a) & (df["world"] == w)]
            )
            for w in worlds
        ]
        for a in agents
    ...
    results = [
        ...
        for a in agents
    ]  # if statement is to prevent empty lists
    scores = [
        statistics.mean([l for aw in a for l in aw])
        if np.nansum([len(r) for r in a])
        else 0
        for a in results
    ]
    stds = [
        statistics.stdev([l for aw in a for l in aw])
        if np.nansum([len(r) for r in a]) > 1
        else 0
        for a in results
    ]
    plt.bar(
    ...
    plt.title(
        "Average pairwise Euclidean distance between runs on same silhouette per agent"
    )
    plt.legend(bbox_to_anchor=(1.04, 0), loc="lower left", borderaxespad=0)
    plt.show()
...
def total_avg_states_evaluated_per_agent(df):
    ...
    scores = [
        [
            np.nansum(run["states_evaluated"])
            for run in get_runs(df[df["agent_attributes"] == a])
        ]
        for a in agents
    ]
...
def illustrate_worlds(df):
    ...
    unique_world_obj = {
        w: df[df["world"] == w].head(1)["_world"].item() for w in unique_world_names
    }
    unique_world_obj = {
        key: value
        for key, value in sorted(unique_world_obj.items(), key=lambda item: item[0])
    }
    plt.figure(figsize=(20, 20))
    for i, (name, world_obj) in enumerate(list(unique_world_obj.items())):
        plt.subplot(
            round(math.sqrt(len(unique_world_obj))),
            round(math.sqrt(len(unique_world_obj))) + 1,
            i + 1,
        )
...
def mean_peak_F1_per_agent_over_worlds(df):
    ...
    for i, (world_name, world_obj) in enumerate(list(unique_world_obj.items())):
        ...
        # all
        results = [
            mean_peak_score(_df[_df["agent_attributes"] == a], scoring_function)
            for a in agents
        ]
        scores = [score for score, std in results]
        stds = [std for score, std in results]
        ...
        results = [
            mean_peak_score(
                _df[(_df["run_ID"].isin(run_IDs)) & (_df["agent_attributes"] == a)],
                scoring_function,
            )
            for a in agents
        ]
...
def mean_failure_reason_per_agent_over_worlds(df, fast_fail=False):
    ...
    for i, (world_name, world_obj) in enumerate(list(unique_world_obj.items())):
        ...
        scores = [
            mean_failure_reason(
                _df[
                    (_df["agent_attributes"] == a)
                    & (_df["world_status"].isin(["Fail", "Ongoing"]))
                ],
                "Full",
            )
            for a in agents
        ...
        if fast_fail:
            # Outside
            scores = [
                mean_failure_reason(_df[_df["agent_attributes"] == a], "Outside")
                for a in agents
            ]
            axes[i, 1].bar(
                np.arange(len(scores)) + 0.45,
            ...
            # Holes
            scores = [
                mean_failure_reason(_df[_df["agent_attributes"] == a], "Holes")
                for a in agents
            ]
            axes[i, 1].bar(
                np.arange(len(scores)) + 0.6,
...
def heatmaps_at_peak_per_agent_over_world(df):
    ...
    fig, axes = plt.subplots(
        len(unique_world_names),
        len(agents) + 1,
        figsize=(2 + 2 * len(agents), 4 + 2 * len(unique_world_names)),
    )
    ...
    for i, (world_name, world_obj) in enumerate(list(unique_world_obj.items())):
        ...
        axes[i, 0].set_yticks([])
        axes[i, 0].set_title(
            textwrap.fill(world_name, width=20), fontsize=10, wrap=True
        )
        # generate heatmaps
        for j, agent in enumerate(agents):
            bms = df[(df["agent_attributes"] == agent) & (df["world"] == world_name)][
                "blockmap"
            ]  # get the correct bms
            if len(bms) == 0:
            ...
            bms = bms.apply(lambda x: (x > np.zeros(shape)) * 1.0)  # make bitmap
            heatmap = np.sum(bms)
            ...
            axes[i, j + 1].set_title(
                textwrap.fill(tit, width=20), fontsize=10, wrap=True
            )
    plt.show()
...
def trajectory_per_agent_over_world(df):
    ...
    fig, axes = plt.subplots(
        len(unique_world_names),
        len(agents) + 1,
        figsize=(2 + 4 * len(agents), 2 + 3 * len(unique_world_names)),
    )
    ...
    for i, (world_name, world_obj) in enumerate(list(unique_world_obj.items())):
        ...
        for j, agent in enumerate(dfic["agent"].unique()):
            img = trajectory.plot_trajectory_graph(
                data=dfic,
...
def scatter_success_cost(df):
    ...
    costs = (
        df.query("final_row == True")
        .groupby("agent_label")["avg_cost_per_step_for_run"]
        .mean()
    )
    ...
    for i in range(len(costs)):
        ...
        plt.scatter(
            costs[i], perfects[i], label=agents[i], marker=get_marker(agents[i])
        )
        axes = plt.gca()
        axes.set_xscale("log")
        plt.annotate(
...
def scatter_success_pairs(df, agent_mappings=None):
    ...
    if agent_mappings is None:
        agent_mappings = generate_pairs(df)
    perfects = df.query("final_row == True").groupby("agent_label")["perfect"].mean()
    plt.plot([[0, 0], [1, 1]], color="grey", alpha=0.4)  # advantage line
    for no_tool_agent, tool_agent, label in agent_mappings:
        plt.scatter(
            perfects[no_tool_agent],
            perfects[tool_agent],
            label=label,
            marker=get_marker(no_tool_agent),
        )
        plt.annotate(
            tool_agent,
...
def scatter_cost_pairs(df, agent_mappings=None):
    ...
    top = max(scores)
    # advantage line
    plt.plot([0, top * 1.1], [0, top * 1.1], color="grey", alpha=0.4)
    for no_tool_agent, tool_agent, label in agent_mappings:
        plt.scatter(
            scores[no_tool_agent],
            scores[tool_agent],
            label=label,
            marker=get_marker(no_tool_agent),
        )
...
def scatter_cost_per_step_pairs(df, agent_mappings=None):
    ...
    if agent_mappings is None:
        agent_mappings = generate_pairs(df)
    scores = (
        df.query("final_row == True").groupby("agent_label")["cost_per_step"].mean()
    )
    top = max(scores)
    bottom = min(scores)
    # advantage line

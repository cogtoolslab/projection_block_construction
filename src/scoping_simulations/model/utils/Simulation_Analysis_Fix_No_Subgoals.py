...
def run():
    ...
    sga = Subgoal_Planning_Agent(
        lower_agent=Best_First_Search_Agent(), decomposer=no_subgoal_decomposer
    )

    simulated_sga = Simulated_Subgoal_Agent(
        decomposer=no_subgoal_decomposer, label="No Subgoals", step_size=-1
    )
    ...
    for i, row in tqdm(list(df.iterrows())):
        ...
        result_sim = simulated_subgoal_planner_experiment_runner.run_experiment(
            result_generator,
            [simulated_sga],
            per_exp=1,
            steps=64,

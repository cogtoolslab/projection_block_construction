...
class MCTS_Agent(BFS_Lookahead_Agent):
    ...
    def __init__(
        self, world=None, horizon=10000, random_seed=None, only_improving_actions=False
    ):
        self.world = world
    ...
    def act(
        ...
        sequence_of_actions = []
        while (
            self.world.status()[0] == "Ongoing" and steps != 0
        ):  # and not cur.is_leaf():
            # choose highest scoring node
            scores = [
                action.target.MC_ratio[1]
                for action in cur.actions
                if action.target.actions is not []
            ]
            if scores == []:
            ...
            action = [
                action for action in cur.actions if action.target.actions is not []
            ][index]
        ...
        return [[b for b in a.action] for a in sequence_of_actions][:step], {
            "states_evaluated": number_of_states_visited
        }
    ...
    class MCTS_Ast_node(Ast_node):
        ...
        def UCT(self, c=math.sqrt(2)):
            ...
            w = self.MC_ratio[0]
            n = (
                self.MC_ratio[1] + sys.float_info[3]
        ...
        def selection(self):
            ...
            while not cur.is_leaf():
                ...
                scored_children = [
                    [action.target.UCT(), action.target] for action in cur.actions
                ]
                max_score = max([a[0] for a in scored_children])
                states_scored += len(scored_children)
                random.seed(self.random_seed)
                cur = random.choice(
                    [a[1] for a in scored_children if a[0] == max_score]
                )
        ...
        def expansion(self):
            ...
            if (
                self.world.is_win(self.state) is True
                or self.world.is_fail(self.state) is True
            ):
        ...
        def simulation(self, max=1000):
            ...
            while w.status()[0] == "Ongoing" and max != 0:
                legal_actions = [
                    a
                    for a in w.possible_actions()
                    if blockworld.legal(w.transition(a)) and w.transition(a).stability()
                ]  # check for stability here
                if legal_actions == []:
                ...
                max = max - 1
            return (
                (True, number_of_states_visited)
                if w.status()[0] == "Win"
                else (False, number_of_states_visited)
            )
        ...
        def add_action(self, action, target=None):
            if action in [act.action for act in self.actions]:
                ...
                if target is None:  # figure out the target state
                    target = MCTS_Agent.MCTS_Ast_node(

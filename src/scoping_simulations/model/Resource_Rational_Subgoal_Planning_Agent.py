...
class Resource_Rational_Subgoal_Planning_Agent(BFS_Lookahead_Agent):
    ...
    def __init__(
        ...
        self.S_threshold = (
            S_treshold  # ignore subgoals that are done in less than this proportion
        )
        self.S_iterations = (
            S_iterations  # how often should we run the simulation to determine S?
        )
        ...
        if decomposer is None:
            try:
                decomposer = (
                    model.utils.decomposition_functions.Horizontal_Construction_Paper(
                        self.world.full_silhouette
                    )
                )
            except (
                AttributeError
            ):  # no world has been passed, will need to be updated using decomposer.set_silhouette
                decomposer = (
                    model.utils.decomposition_functions.Horizontal_Construction_Paper(
                        None
                    )
    ...
    def act(self, steps=1, verbose=False):
        ...
        while cur_i < len(sequence) and cur_i != steps:
            ...
            cur_i += 1
            self.world.set_silhouette(
                self.world.full_silhouette
            )  # restore full silhouette to the world we're acting with
        return lower_level_actions, {
            "states_evaluated": lower_level_cost,
    ...
    def plan_subgoals(self, verbose=False):
        ...
        sequences = self.decomposer.get_sequences(
            ...
            filter_for_length=not self.include_subsequences,
        )
        if verbose:
        ...
        number_of_states_evaluated = self.score_subgoals_in_sequence(
        ...
        return (
            ...
            number_of_states_evaluated,
        )  # return the sequence of subgoals with the highest score
    ...
    def score_sequence(self, sequence):
        ...
        for subgoal in sequence:
            if (
                subgoal.C is None
                or subgoal.S is None
                or subgoal.S < self.S_threshold
                or subgoal.R <= 0
            ):
        ...
        for sequence in sequences:  # reference or copy?
            ...
            for subgoal in sequence:
                ...
                R = self.reward_of_subgoal(
                    subgoal.target, prior_world.current_state.blockmap
                )
                (
                    S,
                    C,
                    ...
                    total_cost,
                    stuck,
                ) = self.success_and_cost_of_subgoal(
                    subgoal.target, prior_world, iterations=self.S_iterations
                )
    ...
    def reward_of_subgoal(self, decomposition, prior_blockmap):
        ...
        return np.sum(
            (decomposition * self.world.full_silhouette) - (prior_blockmap > 0)
        )

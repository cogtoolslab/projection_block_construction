...
class Simulated_Subgoal_Agent(Subgoal_Planning_Agent):
    ...
    def __init__(
        ...
        ):  # note that this will just recreate the behavior of the parent agent
    ...
    def get_parameters(self):
        ...
        try:
            return {
                ...
                **{
                    "parent: " + key: value
                    for key, value in self.parent_agent.get_parameters().items()
                },
            }
        except AttributeError:
            # no parent yet
    ...
    def generate_sequences(self, state=None):
        ...
        self.decomposer.set_silhouette(
            state.world.full_silhouette
        )  # we need to give the decomposer the right silhouette to work with
            ...
            for sequence in self.all_sequences:
                ...
                for i in range(len(seq_names)):
                    if seq_names[i : i + len(target_names)] == target_names:
                        ...
                        if (
                            sequence.subgoals[i].source is None and np.sum(last_bm) == 0
                        ) or (np.all(sequence.subgoals[i].source == last_bm)):
                            Seq = Subgoal_sequence(
                                sequence.subgoals[i : i + len(target_names)]
                            )
                            if (
                                Seq.complete()
                            ):  # do we have a solution to all the subgoals in the sequence?
                                sequences.append(Seq)
                                found = True
                            break
                if found:
                    break  # we've already found a sequence, let's break

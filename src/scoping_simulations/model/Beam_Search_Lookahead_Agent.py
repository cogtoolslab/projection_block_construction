        ...
        while i < self.max_depth:
            ...
            for node in current_nodes:
                ...
                for action in possible_actions:
                    ...
                    target = Ast_node(self.world.transition(action, node.state))
            ...
            if verbose:
                print(
                    "Best 6 actions:",
                    [
                        ",".join([str(a) for a in edge.action])
                        + ", "
                        + str(get_score_edge(edge))
...
def backtrack(state):
    ...
    if state is None:  # if we don't even have a single state
        return []
    while (
        state.parent_action is not None
    ):  # only the root state will have no parent action

...
class Beam_Search_Lookahead_Agent(BFS_Lookahead_Agent):
    ...
    def __init__(self, world=None, beam_width=1000, max_depth=20, heuristic=blockworld.F1_stability_score, only_improving_actions=False, label="Beam Search"):
        self.world = world
    ...
    def __str__(self):
        ...
        return self.__class__.__name__+' beam_width: '+str(self.beam_width)+' heuristic: '+self.heuristic.__name__+' max_depth '+str(self.max_depth) + ' label: ' + self.label
    ...
    def beam_search(self, verbose=False):
        ...
        # contains the states at the current level of depth that we expand upon
        current_nodes = [root]
        while i < self.max_depth:
            ...
            if candidate_edges == []:  # if we can't act any further, end the search
                if verbose:
                    print("No candidate edges, ending with current best node")
                current_nodes.sort(key=get_score_node)
                # return the path to the best current state
                return backtrack(current_nodes[0]), number_of_states_evaluated
            current_nodes = []
            # select the most promising edges

            # helper function to get the score of a target state according to heuristic function
            def get_score_edge(edge):
                return self.heuristic(edge.target.state)

            # helper function to get the score of a target state according to heuristic function
            def get_score_node(node):
                return self.heuristic(node.state)
            # sort the list according to the heuristic
            candidate_edges.sort(key=get_score_edge, reverse=True)
            if verbose:
                print("Best 6 actions:", [",".join([str(a) for a in edge.action])+", "+str(
                    get_score_edge(edge)) for edge in candidate_edges][:6])
        ...
        current_nodes.sort(key=get_score_node)
        if verbose:
            print("Didn't find winning node, returning best so far")
        # return the path to the best current state
        return backtrack(current_nodes[0]), number_of_states_evaluated

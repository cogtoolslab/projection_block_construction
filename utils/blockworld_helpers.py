...
def save_world_render(block_dict,
    ...
    if not os.path.exists(path_to_dump):
        os.makedirs(path_to_dump)
    fig = draw_world(w)
    fig.savefig(os.path.join(path_to_dump, 'blockworld_area{}_num{}_time{}.png'.format(
...
def save_world(block_dict, path_to_dump='./sampled_worlds'):
    ...
    fig = save_world_render(block_dict, worldId=worldId,
                            path_to_dump=path_to_dump + "_render")
...
class World:
    ...
    def __init__(self,
        ...
        self.base_blocks = [BaseBlock(w, h, color=c) for ((w, h), c) in list(zip(sorted(block_dims), block_colors[0:len(
            block_dims)]))]  # Block types should be in order from left to right, thinest to thickest, shortest to tallest
        ...
        self.blockmap = np.zeros(
            (self.world_width, self.world_height), dtype=int)
        self.blocks = []
        self.full = False
    ...
    def check_full(self):
        ...
        '''
        if not self.full:
            self.full = (sum(sum(self.blockmap)) ==
    ...
    def fill_floor_here(self, current_level, left_lim, right_lim):
        ...
        while remaining_space > 0 and len(viable_blocks) > 0:
            ...
            if base_block.width <= remaining_space and base_block.height <= remaining_height:
                b = Block(self.base_blocks[i],
                          floor_block_x_location, current_level)
    ...
    def _update_map_with_blocks(self, blocks, delete=False):
        ...
        for b in blocks:
            self.blockmap[self.world_height -
                          (b.y+b.height): self.world_height-b.y, b.x:(b.x+b.width)] = new_number

    def can_place(self, block):
        overlap = self.blockmap[self.world_height-(
            block.y+block.height): self.world_height-block.y, block.x: (block.x+block.width)]
    ...
    def fill_world(self, render=False):
        ...
        while current_level <= self.world_height - 1:  # Until at top
            ...
            while self.blockmap[self.world_height - current_level - 1].all() and current_level < self.world_height:
                current_level += 1
    ...
    def jenga_block(self, block_number, render=False, checking=False):
        ...
        if block_number < len(self.blocks):
            ...
            new_blockmap[self.world_height -
                         (b.y+b.height): self.world_height-b.y, b.x:(b.x+b.width)] = 0
            if render:
                print(new_blockmap)
